...
export const sendForgotPasswordEmail = async (to, otp) => {
  try {
    ...
    sendSmtpEmail.subject = "Reset your password - EduExam";
    sendSmtpEmail.htmlContent = `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
          <h2 style="color: #2a384a;">Password Reset</h2>
          <p>Your OTP code to reset your EduExam password is:</p>
          <div style="background-color: #f4f4f4; padding: 15px; font-size: 24px; font-weight: bold; text-align: center; border-radius: 5px; letter-spacing: 5px;">
            ${otp}
          </div>
          <p>This code is valid for 10 minutes.</p>
    ...
    const data = await apiInstance.sendTransacEmail(sendSmtpEmail);
    console.log("✅ Brevo Password Reset Email sent successfully:", data);
    return data;
  } catch (error) {
    console.error("❌ Brevo Email Error:", error.response ? error.response.body : error.message);
    throw error;
